...
Telepítse az eszközt. Az utasításokért lásd: [Az Azure Rights Managementhez készült Windows PowerShell telepítése](../deploy-use/install-powershell.md)..
...
> [!NOTE]
> További információk a hardveres védelem AD RMS szolgáltatással történő használatáról: [Using AD RMS with Hardware Security Modules](http://technet.microsoft.com/library/jj651024.aspx) (Az AD RMS használata hardveres biztonsági modulokkal)..
...
|-----------------------------|----------------------------------------|--------------------------|
|Jelszavas védelem az AD RMS adatbázisban|Microsoft által felügyelt|Lásd a **Szoftveres védelemmel ellátott kulcs áttelepítése szoftveres védelemmel rendelkező kulccsá** eljárást a táblázat után.<br /><br />Ez a legegyszerűbb áttelepítési út. Mindössze annyit igényel, hogy átvigye a konfigurációs adatait az Azure RMS-be.|
|HSM-es védelem Thales nShield hardveres biztonsági modul (HSM) használatával|Felhasználó által felügyelt (BYOK)|Lásd a **HSM által védett kulcs áttelepítése HSM által védett kulccsá** eljárást a táblázat után.<br /><br />Ehhez szükséges a BYOK eszközkészlet, valamint két további lépés a kulcs átviteléhez a helyszíni HSM-ből az Azure RMS HSM-ekbe, majd a konfigurációs adatok átviteléhez az Azure RMS-be.|
...
Továbbá, ha a sablonok az AD RMS-ben a **BÁRKI** csoportot használták, a rendszer automatikusan eltávolítja ezt a csoportot, amikor a sablonokat importálja az Azure RMS-be. Az importált sablonokhoz manuálisan kell hozzáadni az egyenértékű csoportot vagy felhasználókat és az azonos jogosultságokat. Az egyenértékű csoport neve az Azure RMS esetében: **AllStaff-7184AB3F-CCD1-46F3-8233-3E09E9CF0E66@<bérlő_neve>.onmicrosoft.com**. Ez a csoport például a következőképpen nézheti ki a Contoso esetében: **AllStaff-7184AB3F-CCD1-46F3-8233-3E09E9CF0E66@contoso.onmicrosoft.com**.

Ha nem biztos benne, hogy az AD RMS-sablonjai tartalmazzák a BÁRKI csoportot, a PowerShell-parancsfájlpélda használatával azonosíthatja ezeket a sablonokat. További információk a Windows PowerShell AD RMS szolgáltatással történő használatáról: [Using Windows PowerShell to Administer AD RMS](https://technet.microsoft.com/library/ee221079%28v=ws.10%29.aspx) (Az AD RMS felügyelete a Windows PowerShell használatával)..
...
Az AD RMS szolgáltatásból importált sablonok kinézete és viselkedése pont olyan, mint a klasszikus Azure-portálon létrehozottaké. Ha szeretné módosítani az importált sablonok közzétételét, hogy a felhasználók lássák és az alkalmazásokból kiválaszthassák azokat, vagy más módosításokat szeretne végrehajtani a sablonokon, olvassa el az [Egyéni sablonok konfigurálása az Azure Rights Management szolgáltatáshoz](../deploy-use/configure-custom-templates.md) című témakört..

> [!TIP]
> Az áttelepítési folyamat során tapasztalt egységesebb felhasználói élmény érdekében ekkor még ne hajtson végre más módosítást az importált sablonokon ezen a két módosításon kívül, és ne tegye közzé az Azure RMS szolgáltatáshoz tartozó két alapértelmezett sablont, illetve ne hozzon létre új sablont. Inkább várja meg az áttelepítési folyamat befejezését és az AD RMS-kiszolgálók leszerelését.
