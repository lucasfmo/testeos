...
# A szolgáltatásalkalmazás alkalmassá tétele a felhőalapú RMS használatára

Ez a témakör a szolgáltatásalkalmazás Azure Rights Management használatához végzett beállításának lépéseit ismerteti. További információ: [Ismerkedés az Azure Rights Management szolgáltatással](https://technet.microsoft.com/en-us/library/jj585016.aspx).

**Fontos**  
Először ajánlott a Rights Management Services SDK 2.1 alkalmazás az éles üzem előtti RMS környezetben egy RMS-kiszolgálóval tesztelni. Ha ezután azt szeretné, hogy az ügyfél használhassa az alkalmazást az Azure RMS szolgáltatással, folytassa a tesztelést ezzel a környezettel.

Ha az RMS SDK 2.1 szolgáltatásalkalmazást az Azure RMS rendszerrel szeretné használni, Azure RMS-bérlőt kell kérnie, ha még nem rendelkezik azzal. A bérlőkérést küldje el e-mailben a <rmcstbeta@microsoft.com> címre.
...
**Megjegyzés** Az észlelési szolgáltatás jelenlegi feltételei miatt Észak-Amerikában kívül a szimmetrikus kulcs hitelesítő adatai nem fogadhatók el más régiókból, így közvetlenül kell megadnia a bérlő URL-címeket. Ezt az [**IpcGetTemplateList**](/rights-management/sdk/2.1/api/win/functions#msipc_ipcgettemplatelist) vagy [**IpcGetTemplateIssuerList**](/rights-management/sdk/2.1/api/win/functions#msipc_ipcgettemplateissuerlist) [**IPC\_CONNECTION\_INFO**](/rights-management/sdk/2.1/api/win/ipc_connection_info#msipc_ipc_connection_info) paraméterén keresztül teheti meg.
...
-   Hozza létre az [**IPC\_CREDENTIAL\_SYMMETRIC\_KEY**](/rights-management/sdk/2.1/api/win/ipc_credential#msipc_ipc_credential_symmetric_key) példányát, és állítson be néhány tagot.
    ...
    symKey.wszBase64Key = "a szolgáltatás elsődleges kulcsa";
    symKey.wszAppPrincipalId = "az alkalmazás elsődleges azonosítója";
    symKey.wszBposTenantId = "a bérlő azonosítója";


További információkért lásd: [**IPC\_CREDENTIAL\_SYMMETRIC\_KEY**](/rights-management/sdk/2.1/api/win/ipc_credential#msipc_ipc_credential_symmetric_key).
...
**Megjegyzés** A *conectionInfo* tagok a `Get-AadrmConfiguration` előző hívásából származó URL-ekkel vannak beállítva és itt láthatók ezekkel a mezőnevekkel.
